...
module.exports = {
  ...
  viewDashboard: async (req, res) => {
    try {
      ...
      res.render('admin/dashboard/view_dashboard', {
        title: 'Staycation | Holiday Anytime',
  ...
  deleteCategory2: async (req, res) => {
    ...
    try {
      ...
      for (let i = 0; i < item.categoryId.length; i++) {
        if (item.categoryId[i]._id.toString() === category._id.toString()) {
          item.categoryId.pull({ _id: category._id });
          await item.save();
        }
      }
      await category.remove();
  ...
  viewItem: async (req, res) => {
    try {
      ...
      // console.log(item);
      const category = await Category.find();
      // const category = await Category.find().populate('itemId');
      console.log(category);
      const alertMessage = req.flash('alertMessage');
      const alertStatus = req.flash('alertStatus');
      const alert = { message: alertMessage, status: alertStatus };
  ...
  deleteItem2: async (req, res) => {
    ...
    try {
      ...
      for (let y = 0; y < category.itemId.length; y++) {
        if (category.itemId[y]._id.toString() === item._id.toString()) {
